...
const EmployeeForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const employeeData: Employee = {
        ...
        image: formData.image, // This will be the base64 string from ImageCropper
        updatedAt: new Date()
      };

      if (isEditing && selectedEmployee?._id) {
        updateEmployee(selectedEmployee._id, employeeData);
        addNotification({
          type: 'success',
          title: 'Employee Updated',
          message: `${employeeData.fullName} has been updated successfully.`
        });
      } else {
        ...
        addNotification({
          ...
          message: `${employeeData.fullName} has been added successfully.`
        });
      }
  ...
  return (
    <Dialog open={employeeFormDialogOpen} onOpenChange={closeEmployeeFormDialog}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Contact Information */}
            <FormSection title="Contact Information" icon={<MapPin className="h-4 w-4" />}>
            ...
            {/* Personal Details */}
            <FormSection title="Personal Details" icon={<IdCard className="h-4 w-4" />}>
              <div className="space-y-4">
                <NICInput
                  label="NIC Number *"
            ...
            <FormSection title="Employment Information" icon={<Briefcase className="h-4 w-4" />}>
              <div className="space-y-4">
                ...
                {/* Grade Appointment Dates */}
                <div>
                  <Label>Grade Appointment Dates</Label>
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    <DateInput
                      label="Grade III"
                      value={formData.gradeAppointmentDate.gradeIII || ''}
                      onChange={(value) => handleFieldChange('gradeAppointmentDate.gradeIII', value)}
                    />
                    <DateInput
                      label="Grade II"
                      value={formData.gradeAppointmentDate.gradeII || ''}
                      onChange={(value) => handleFieldChange('gradeAppointmentDate.gradeII', value)}
                    />
                    <DateInput
                      label="Grade I"
                      value={formData.gradeAppointmentDate.gradeI || ''}
                      onChange={(value) => handleFieldChange('gradeAppointmentDate.gradeI', value)}
                    />
                    <DateInput
                      label="Grade Supra"
                      value={formData.gradeAppointmentDate.gradeSupra || ''}
                      onChange={(value) => handleFieldChange('gradeAppointmentDate.gradeSupra', value)}
                    />
                  </div>
                </div>
            ...
            <FormSection title="Additional Information" icon={<FileText className="h-4 w-4" />}>
              <div className="space-y-4">
                <div>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={closeEmployeeFormDialog}>
