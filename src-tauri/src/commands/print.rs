...
pub async fn backup_monthly_data(
    ...
    let db = &state.db;
    let attendance_collection = db.collection::<AttendanceRecord>(Collections::ATTENDANCE);
    let leaves_collection = db.collection::<LeaveApplication>(Collections::LEAVES);
    ...
    fs::write(&attendance_file, attendance_json)
        .map_err(|e| format!("Failed to write attendance backup: {}", e))?;

    // Backup leave applications
    let leave_start = format!("01-{:02}-{}", month, year);
    let leave_end = if month == 12 {
        format!("31-12-{}", year)
    } else {
        format!("31-{:02}-{}", month, year)
    };

    let leaves_filter = doc! {
        "appliedDate": {
            "$gte": leave_start,
            "$lte": leave_end
        }
    ...
    let mut leaves_cursor = leaves_collection.find(leaves_filter, None)
        .await
        .map_err(|e| format!("Failed to find leave applications: {}", e))?;

    let mut leave_applications = Vec::new();
    while let Some(application) = leaves_cursor.try_next()
        .await
        .map_err(|e| format!("Failed to iterate leave applications: {}", e))? {
        leave_applications.push(application);
    }

    let leaves_file = backup_dir.join("leaves.json");
    let leaves_json = serde_json::to_string_pretty(&leave_applications)
        .map_err(|e| format!("Failed to serialize leave data: {}", e))?;
    fs::write(&leaves_file, leaves_json)
        .map_err(|e| format!("Failed to write leaves backup: {}", e))?;
...
pub async fn clear_monthly_data(
    ...
    let leaves_result = leaves_collection.delete_many(leaves_filter, None)
        .await
        .map_err(|e| format!("Failed to delete leave applications: {}", e))?;

    Ok(PrintResponse {
        success: true,
        file_path: None,
        message: Some(format!(
            "Cleared {} attendance records and {} leave applications for {}/{}",
            attendance_result.deleted_count,
            leaves_result.deleted_count,
            month,
            year
        )),
        error: None,
